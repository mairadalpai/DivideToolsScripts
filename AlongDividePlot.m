function AlongDividePlot(head_vals,varargin)
    ...
    mean_d = mean([-1*dlt(:,2);-1*dlt(:,5);dlt(:,4); dlt(:,3)]);
    std_d = std([-1*dlt(:,2);-1*dlt(:,5);dlt(:,4); dlt(:,3)]);
    subplot(1,1,1)
    hold on
    plot([0,num_divides+1],[0,0],'--k');
    e = errorbar(dlt(:,1),(-1*dlt(:,2)-mean_d)/std_d,dlt(:,6),'-g');
    e.LineStyle = 'none';
    scatter(dlt(:,1),(-1*dlt(:,2)-mean_d)/std_d,20,'g','filled');
	xlim([0,num_divides+1]);
	xticks([1:1:num_divides]);
	xticklabels(tick_names);
	ax1=gca;
    ...
    hold on
    plot([0,num_divides+1],[0,0],'--k');
    e = errorbar(dlt(:,1)+0.1,(-1*dlt(:,5)-mean_d)/std_d,dlt(:,9),'-b');
    e.LineStyle = 'none';
    scatter(dlt(:,1)+0.1,-1*dlt(:,5),20,'b','filled');
	xlim([0,num_divides+1]);
	xticks([1:1:num_divides]);
	xticklabels(tick_names);
	ax1=gca;
    ...
    hold on
	plot([0,num_divides+1],[0,0],'--k');
    e = errorbar(dlt(:,1)+0.2,(dlt(:,3)-mean_d)/std_d,dlt(:,7),'-r');
    e.LineStyle = 'none';
    scatter(dlt(:,1)+0.2,(dlt(:,3)-mean_d)/std_d,20,'r','filled');
	xlim([0,num_divides+1]);
	xticks([1:1:num_divides]);
	xticklabels(tick_names);
	ax1=gca;
    ...
    hold on
    plot([0,num_divides+1],[0,0],'--k');
    e = errorbar(dlt(:,1)+0.3,(dlt(:,4)-mean_d)/std_d,dlt(:,8),'-c');
    e.LineStyle = 'none';
    scatter(dlt(:,1)+0.3,(dlt(:,4)-mean_d)/std_d,20,'c','filled');
	xlim([0,num_divides+1]);
	xticks([1:1:num_divides]);
	xticklabels(tick_names);
	ax1=gca;
